        ...
        <div class="card-content">
          ...
          <div class="action-group dual">
            ...
            <button
              ...
              :disabled="phase < 4"
              @click="existingSimStatus ? handleViewSimulation() : handleStartSimulation()"
            >
              {{ existingSimStatus ? $t('step2.viewSimulation', 'View Simulation') : $t('step2.startDualWorldSim') }} ➝
            </button>
          </div>
        </div>
      </div>
...
const existingSimStatus = ref(null) // null, 'running', 'completed', 'stopped', 'failed'
...
const handleViewSimulation = () => {
  addLog('Navigating to existing simulation...')
  emit('next-step', {})
}
...
onMounted(async () => {
  ...
  if (props.simulationId) {
    ...
    // Check if a simulation run already exists
    try {
      const res = await getRunStatus(props.simulationId)
      if (res.success && res.data && res.data.runner_status) {
        existingSimStatus.value = res.data.runner_status
      }
    } catch (err) {
