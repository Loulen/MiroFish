...
def get_locale() -> str:
    if has_request_context():
        ...
        return raw if raw in _translations else 'fr'
    return getattr(_thread_local, 'locale', 'fr')


def t(key: str, **kwargs) -> str:
    locale = get_locale()
    messages = _translations.get(locale, _translations.get('fr', {}))
    ...
    if value is None:
        value = _translations.get('fr', _translations.get('en', {}))
        for part in key.split('.'):
            if isinstance(value, dict):
                value = value.get(part)
            else:
...
def get_language_instruction() -> str:
    locale = get_locale()
    lang_config = _languages.get(locale, _languages.get('fr', {}))
    return lang_config.get('llmInstruction', 'Veuillez répondre en français.')
